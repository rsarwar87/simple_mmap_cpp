...
constexpr std::array<std::tuple<uintptr_t, uint32_t, uint32_t, uint32_t>, count>
    memory_array = {{std::make_tuple(spi_addr, spi_range,
                                     PROT_READ | PROT_WRITE, spi_nblocks),
                     std::make_tuple(gpio_addr, gpio_range,
                                     PROT_READ | PROT_WRITE, gpio_nblocks),
                     std::make_tuple(xadc_addr, xadc_range,
                                     PROT_READ | PROT_WRITE, xadc_nblocks),
                     std::make_tuple(adc_addr, adc_range,
                                     PROT_READ | PROT_WRITE, adc_nblocks),
                     std::make_tuple(dac_addr, dac_range,
                                     PROT_READ | PROT_WRITE, dac_nblocks)}};
...
constexpr uint32_t CHIP_SELECT = 0x70;
}  // namespace spi
